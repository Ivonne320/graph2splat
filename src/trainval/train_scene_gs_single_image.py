...
from src.datasets import Scan3RSceneBatchSingleDataset
...
class Trainer(EpochBasedTrainer):
    def __init__(self, cfg: Config, parser: argparse.ArgumentParser = None) -> None:
        ...
        dataset_cls = Scan3RSceneBatchSingleDataset
    ...
    def create_model(self) -> LatentAutoencoder:
        ...
        #     torch.load(
        #         "/cluster/scratch/wangyih/overfitting_dataset/pretrained/debug_gs/100scenes/with-encoder-clamp-min-scale-con-full-100loss/snapshots/epoch-44.pth.tar", map_location=self.device
        #     )["model"]
        # )
    ...
    def _clamp_gaussian_scale(self, reconstruction: Gaussian, bbox_scale: torch.Tensor) -> None:
        ...
        max_scale = 1.0 * (bbox_scale / 128).clamp_min(1e-7)
    ...
    def _train_step_scene(
        ...
        scene_ids = [sid[0] for sid in data_dict["scene_graphs"]["scene_ids"]]
        frame_ids = data_dict["scene_graphs"].get("frame_ids", None)
        # frame_ids is a list aligned with scene_ids (length == batch_size)
        ...
        for b, sid in enumerate(scene_ids):
            ...
            fid = frame_ids[b] if frame_ids is not None else None
            if fid is None:
                continue
            ...
            for fid in fids:
                ...
                # l1_terms.append(l1_loss(rendered, image)/(mask.mean() + 1e-6))
