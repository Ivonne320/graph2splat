...
class BaseTrainer(abc.ABC):
    def __init__(
        ...
        if not torch.cuda.is_available():
            _LOGGER.warning("No CUDA devices available.")
        self.device = (
        ...
        torch_util.initialize(
            self.seed, self.cudnn_deterministic, self.autograd_anomaly_detection
        )
    ...
    def load_snapshot(self, snapshot, fix_prefix=True):
        ...
        if self.args.load_encoder == "sg_aligner":
            model_dict = OrderedDict(
                [("encoder." + key, value) for key, value in model_dict.items()]
            )
        elif self.args.load_encoder == "cross_loc":
            model_dict = OrderedDict(
                [
                    (key.replace("sg_encoder", "encoder"), value)
    ...
    def release_tensors(self, result_dict):
        ...
        if self.distributed:
            result_dict = torch_util.all_reduce_tensors(
                result_dict, world_size=self.world_size
            )
    ...
    def write_event(self, phase, event_dict, index):
        ...
        for key, value in event_dict.items():
            self.writer.add_scalar(f"{phase}/{key}", value, global_step=index)
