...
#         # self.rotation_activation = lambda x: F.normalize(x, dim=-1, eps=1e-6)
...
#         )
    ...
    #             setattr(self, attr, value.to(target))

    #     self.aabb = self.aabb.to(target)
    #     self.scale_bias = self.scale_bias.to(target)
    #     self.rots_bias = self.rots_bias.to(target)
    #     self.opacity_bias = self.opacity_bias.to(target)
    #     self.device = target
    #     return self
...
#         inner = (scales**2 - self.mininum_kernel_size**2).clamp_min(1e-9)
#         scales = torch.sqrt(inner)
...
#         )
#     # def rescale(self, scaling):
#     #     self._xyz = self._xyz * scaling

#     #     target_scale = self.get_scaling * scaling
#     #     self.from_scaling(target_scale)
