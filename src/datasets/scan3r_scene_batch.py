...
class Scan3RSceneBatchDataset(data.Dataset):
    def __init__(self, cfg: Config, split: str):
        ...
        _LOGGER.info(f"use_student_structure={self.use_student_structure}")
        student_cfg = getattr(cfg.data, "student_structure", None)
        default_root = "/cluster/scratch/wangyih/3RScan/files/gs_annotations"
        self.student_pack_root = (
            getattr(student_cfg, "root", default_root)
        ...
        self.force_frame_id = getattr(cfg.data, "force_frame_id", None)
        if self.force_frame_id is not None:
            self.force_frame_id = str(self.force_frame_id).zfill(6)
        ...
        self._dino_token_cache: dict[tuple[str, str], torch.Tensor] = {}
    ...
    def generate_data_items(self) -> list:
        ...
        for scan_id in self.scan_ids:
            image_paths = self.image_paths[scan_id]
            frame_indices = sorted(image_paths.keys())
            if self.structure_frame_cap is not None:
                frame_indices = frame_indices[: self.structure_frame_cap]
            if self.force_frame_id is not None:
                if self.force_frame_id in image_paths:
                    frame_indices = [self.force_frame_id]
                else:
                    continue
            # i = 0                        # <- remove
    ...
    def _collate(self, batch: list) -> dict:
        ...
        # scene_graphs_['masks'] = self._aggregate(
        #     splat_dict, "masks", "torch_cat"
        # ).float()
        scene_graphs_["R_cans"] = self._aggregate(
            splat_dict, "R_cans", "torch_cat"
        ).float()
    ...
    def collate_fn(self, batch: list) -> dict:
        scans_batch = [b["scan_id"] for b in batch]
        data_dict = {}
        ...
        # Intrinsics per scene
        obj_intrinsics = {sid: self.image_intrinsics[sid] for sid in scans_batch}
        scene_graphs_["obj_intrinsics"] = obj_intrinsics
        ...
        if self.use_student_structure:
            scene_graphs_["student_gt_indices"] = [
                d["student_gt_indices"] for d in splat_dicts
            ]
            scene_graphs_["student_seed_indices"] = [
                d["student_seed_indices"] for d in splat_dicts
            ]
            scene_graphs_["student_aligned_feats"] = [
                d["student_aligned_feats"] for d in splat_dicts
            ]
            scene_graphs_["student_grid_resolution"] = [
                d["student_grid_resolution"] for d in splat_dicts
            ]
