...
class SummaryBoard:
    ...
    def full_summary(self):
        summary_dict = {}
        for name in self.meter_names:
            summary_dict[f"{name}_mean"] = self.meter_dict[name].mean()
            summary_dict[f"{name}_std"] = self.meter_dict[name].std()
            summary_dict[f"{name}_median"] = self.meter_dict[name].median()
        return summary_dict
