...
class MaskedInpaintingEvaluator:
    def __init__(
        self,
        cfg: Config,
        split: str = "val",
        ...
        self.model = MaskedInpaintingUNet(in_channels=2, base_channels=base_ch).to(self.device).eval()
    ...
    def _load_checkpoint(self, checkpoint: Optional[str]) -> None:
        ...
        if checkpoint is None or not osp.exists(checkpoint):
            raise ValueError(
                "Checkpoint not provided. Use --checkpoint or cfg.inference.inpainting_path."
            )
    ...
    def _indices_to_occ(self, idx_t: torch.Tensor, G: int) -> torch.Tensor:
        occ = torch.zeros(1, 1, G, G, G, dtype=torch.float32)
        if idx_t.numel() > 0:
    ...
    def _process_batch(
        ...
        occ_gt, seed_occ, mask = self._make_batch(batch)
        B = occ_gt.shape[0]
        x_in = torch.cat([seed_occ, mask], dim=1).to(self.device)
        ...
        if self.save_vis:
            self._dump_visuals(scene_ids, occ_gt, logits, seed_occ)
    ...
    def _make_batch(self, data_dict: Dict[str, Any]):
        ...
        for idx, sid in enumerate(scene_ids):
            ...
            try:
                ...
                continue
            mean_gt_t = mean_gt.unsqueeze(0)
            scale_gt_t = scale_gt.view(1, 1)
            mean_seed0_t = mean_seed0.unsqueeze(0)
            scale_seed0_t = scale_seed0.view(1, 1)
            seed_idx_t = torch.from_numpy(seed_idx).int()
            if seed_idx_t.numel() > 0:
                idx_remap = self._remap_seed_idx_with_bbox(
                    seed_idx_t, mean_seed0_t, scale_seed0_t, mean_gt_t, scale_gt_t, self.G, b=0
                )
            else:
                idx_remap = seed_idx_t
            seed_occ = self._indices_to_occ(idx_remap, self.G)
            occ_gt_list.append(occ_gt.unsqueeze(0))
            seed_occ_list.append(seed_occ)
            mask_list.append((seed_occ > 0).float())

        if not occ_gt_list:
            raise RuntimeError("No valid packs found in batch during evaluation.")
        occ_gt = torch.cat(occ_gt_list, dim=0).to(self.device)
        seed_occ = torch.cat(seed_occ_list, dim=0).to(self.device)
        mask = torch.cat(mask_list, dim=0).to(self.device)
        return occ_gt, seed_occ, mask
...
def parse_args():
    ...
    parser.add_argument("--checkpoint", type=str, default=None, help="Masked inpainting checkpoint path")
...
def main():
    ...
    evaluator = MaskedInpaintingEvaluator(
        cfg,
        split=args.split,
