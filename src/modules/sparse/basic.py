...
if BACKEND == "torchsparse":
    ...
    print("Using torchsparse backend")
elif BACKEND == "spconv":
    SparseTensorData = importlib.import_module("spconv.pytorch").SparseConvTensor
    print("Using spconv backend")
...
def sparse_batch_cat(inputs: List[SparseTensor]) -> SparseTensor:
    """
    Concatenate a list of sparse tensors along the batch dimension.
    Each sparse tensor has N points and C features.
    Each coordinate will be expanded to [N, 4] by adding batch index.

    Note: coords will be expanded from [N, 3] to [N, 4] by adding batch index.
    ...
    coords = torch.cat(
        [
            torch.cat([torch.full((input.coords.shape[0], 1), i), input.coords], dim=1)
