...
def voxelise_features(
    ...
    mean, scale, pts_normed = normalize_points_with_padding(world_points, G=64, pad_ratio=1.5, isotropic=False)
    ...
    coords = vox_idx[idx_keep_1]
    payload = np.concatenate([coords.astype(np.float32), tokens_1.astype(np.float32)], axis=1)
    vis.save_voxel_as_ply(payload,
                          "vis/single_image_voxel_structure_model_inference.ply",
                          show_color=True)
    _save_featured_voxel(torch.from_numpy(payload), output_file=voxel_path)
    seed_idx = vox_idx[idx_keep_1]
    pack = {
        "G": np.int32(G),
        "mean": mean.astype(np.float32),
        "scale": scale.astype(np.float32),
        "seed_idx": seed_idx.astype(np.int32),
        "feats": tokens_1.astype(np.float32),               # aligned with seed_idx
        "frame_id_used": np.array(frame_id),
        "pad_meta": np.array([-1, 1.5], dtype=np.float32),
    }
    out_name = f"student_pack_aligned_{fid0}.npz"
