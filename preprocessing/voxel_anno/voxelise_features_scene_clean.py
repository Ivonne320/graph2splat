...
def _get_dino_embedding(images: torch.Tensor, batch_size: int = 4) -> torch.Tensor:
    all_embeddings = []
    # Process in batches to save VRAM
    for i in range(0, images.shape[0], batch_size):
        batch = images[i : i + batch_size].cuda()
        # Transform should be applied to the batch
        ...
        all_embeddings.append(patch_embeddings.cpu()) # Move to CPU to clear GPU
...
def _normalize_segmented_mesh(segmented_mesh: o3d.geometry.TriangleMesh):
    ...
    # _LOGGER.info(f"mean:{mean}, scale:{scale}")
...
def voxelise_features(
    ...
    rendered = []
    for frame_id in frame_idxs:
        img_path = f"{root_dir}/scenes/{scan_id}/sequence/frame-{frame_id}.color.jpg"
        with Image.open(img_path) as im:
            # im = im.convert("RGB")  # ensure 3-channel
            rendered.append(torch.from_numpy(np.array(im)).permute(2, 0, 1).float() / 255.0)
    # rendered = [
    ...
    try:
        ...
        pcd = scene_mesh.sample_points_uniformly(number_of_points=400000)
        ...
        num_images = patch_embeddings.shape[0]
        num_voxels = voxel_grid.shape[0]
        accumulated_features = torch.zeros((num_voxels, 1024), dtype=torch.float32)
        for i in tqdm(range(num_images), desc="Projecting frames"):
            # Move only one frame's data to GPU at a time
            feat_frame = patch_embeddings[i:i+1].cuda()
            proj_frame = projection[i:i+1].cuda().unsqueeze(1) # (1, 1, N, 2)
            ...
            # Squeeze and add to total
            accumulated_features += sampled.squeeze().permute(1, 0).cpu()
            
            # Clear cache to prevent fragmentation
            del feat_frame, proj_frame, sampled
        patchtokens = (accumulated_features / num_images).numpy().astype(np.float16)
