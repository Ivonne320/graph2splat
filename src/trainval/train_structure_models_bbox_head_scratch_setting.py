...
class Trainer(EpochBasedTrainer):
    ...
    def create_model(self) -> StructureModel:
        ...
        #     torch.load(
        #         "/cluster/scratch/wangyih/overfitting_dataset/pretrained/training_structure_model/2025-11-5_100_scenes_continue_debug_Tversky_bce_focal/snapshots/epoch-2.pth.tar", map_location=self.device
        #     )["model"]
        # )
        # model.load_state_dict(
        #     torch.load(
    ...
    def _make_batch(self, data_dict: Dict[str, Any]):
        ...
        for idx, sid in enumerate(scene_ids):
            ...
            occ_gt_list.append(occ_gt.unsqueeze(0))          # (1,1,G,G,G)
            seed_idx_list.append(seed_idx)                   # (Mi,3) np.int32
            feats_list.append(feats)                         # (Mi,1024) np.float32
            mean_gt_list.append(mean_gt.unsqueeze(0))        # (1,3)
            scale_gt_list.append(scale_gt.unsqueeze(0))      # (1,1)
    ...
    def train_step(
        ...
        # Swapping tversky
        # alpha, beta = 0.3, 0.7
        # P = probs*mask_complete; G = gt*mask_complete
        # TP = (P*G).sum((1,2,3,4))
        ...
        lambda_dice = 4
        loss = recon_w * (vae_rec  + lambda_dice * dice_loss)
        # loss = 10 * vae_rec
        with torch.no_grad():
            probs = torch.sigmoid(logits32)                 # (B,1,G,G,G)
            gt    = (occ_gt32 > 0.5).float()                # binarize GT once

            def batch_iou_at(thr: float) -> torch.Tensor:
                pr = (probs >= thr).float()
                inter = (pr * gt).sum(dim=(1,2,3,4))        # (B,)
                union = pr.sum(dim=(1,2,3,4)) + gt.sum(dim=(1,2,3,4)) - inter
                return (inter / (union + 1e-6)).mean()      # mean over batch

            iou_03 = batch_iou_at(0.30)
            iou_05 = batch_iou_at(0.50)
            iou_07 = batch_iou_at(0.70)
