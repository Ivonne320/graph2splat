...
class Scan3RSceneBatchDataset(data.Dataset):
    ...
    def collate_fn(self, batch: list) -> dict:
        ...
        for sid in scans_batch:
            if load_single:
                splat_dicts.append(self._load_splats_single_frame(
                    sid, preload_slat=self.cfg.data.preload_slat
                ))
            else:
                splat_dicts.append(self._load_splats(
                    sid, preload_slat=self.cfg.data.preload_slat
                ))

        from src.modules.sparse.basic import sparse_cat
        scene_graphs_["tot_obj_splat"] = sparse_cat([d["tot_obj_splat"] for d in splat_dicts]).float()
        scene_graphs_["mean_obj_splat"] = torch.stack([d["mean_obj_splat"] for d in splat_dicts]).float()
        scene_graphs_["scale_obj_splat"] = torch.stack([d["scale_obj_splat"] for d in splat_dicts]).float()
        scene_graphs_["R_cans"] = torch.stack([d["R_cans"] for d in splat_dicts]).float()
