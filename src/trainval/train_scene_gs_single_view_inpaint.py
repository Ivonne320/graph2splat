...
class Trainer(EpochBasedTrainer):
    def __init__(self, cfg: Config, parser: argparse.ArgumentParser = None) -> None:
        ...
        # if not getattr(self.cfg.data, "use_student_structure", False):
        self.cfg.data.use_student_structure = True
        sv_frames = getattr(loss_cfg, "single_view_supervision_frames", 8)
        if not hasattr(self.cfg.data, "single_view_supervision_frames"):
            self.cfg.data.single_view_supervision_frames = sv_frames
    ...
    def create_model(self) -> LatentAutoencoder:
        ...
        #     torch.load(
        #         "/cluster/scratch/wangyih/overfitting_dataset/pretrained/debug_gs/batch/100scenes_modified_rescale_mask_no_dilation_add_back_loss_clamp_softer/snapshots/epoch-68.pth.tar", map_location=self.device
        #     )["model"]
        # )
        # model.load_state_dict(
        #     torch.load(
        #         "/cluster/scratch/wangyih/overfitting_dataset/pretrained/debug_gs/batch/100scenes_modified_rescale_mask_no_dilation_add_back_loss_clamp_softer/snapshots/epoch-116.pth.tar", map_location=self.device
        #     )["model"]
        # )
        model.load_state_dict(
            torch.load(
                "/cluster/scratch/wangyih/overfitting_dataset/pretrained/debug_gs/batch/debug/20251218_summary_single_scene_resolution/128_reso_higher_lr/snapshots/epoch-870.pth.tar", map_location=self.device
            )["model"]
        )
    ...
    def _clamp_gaussian_scale(self, reconstruction: Gaussian, bbox_scale: torch.Tensor) -> None:
        ...
        max_scale = (bbox_scale / 128.0).clamp_min(1e-5)
    ...
    def train_step(
        ...
        for b, sid in enumerate(scene_ids):
            ...
            R_h[:3, :3] = R_can
            # self.logger.info(f"sid: {sid}, fids: {fids}")
        ...
        loss = (protometric_loss +  10000 * volume_loss + 0.001 * opacity_loss + perceptual_loss) \
