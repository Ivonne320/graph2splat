...
from src.models.latent_autoencoder_decoder_only import LatentAutoencoderDecoderOnly
try:
...
class Trainer(EpochBasedTrainer):
    def __init__(self, cfg: Config, parser: argparse.ArgumentParser = None) -> None:
        ...
        self.volume_loss_weight: float = getattr(
            ...
            # cfg.train.loss, "volume_loss_weight", 1000
        )
    ...
    def create_model(self) -> LatentAutoencoderDecoderOnly:
        if self.cfg.autoencoder.guidance:
            ...
            model = LatentAutoencoderDecoderOnly(
                cfg=self.cfg.autoencoder,
                device=self.device,
                text_guidance=self.text_guidance,
            )
        else:
            model = LatentAutoencoderDecoderOnly(cfg=self.cfg.autoencoder, device=self.device)

        model.load_state_dict(
            torch.load(
                "/cluster/scratch/wangyih/overfitting_dataset/pretrained/debug_gs/100scenes/decoder-only-clamp-min-scale-con-100scenes/snapshots/epoch-294.pth.tar", map_location=self.device
            )["model"]
        )
        # model.load_state_dict(
        #     torch.load(
    ...
    def _clamp_gaussian_scale(self, reconstruction: Gaussian, bbox_scale: torch.Tensor) -> None:
        ...
        max_scale = 1.0 * (bbox_scale / 128.0).clamp_min(1e-7)
    ...
    def _train_step_scene(
        ...
        for b, sid in enumerate(scene_ids):
            ...
            for fid in fids:
                ...
                xyz = recon_b.get_xyz
                scaling = recon_b.get_scaling
                opacity = recon_b.get_opacity
                ...
                # self.logger.info(f"rendered min: {rendered.min()}, rendered max: {rendered.max()}")
    ...
    def _train_step_object(
        ...
        assert self.model is not None and isinstance(self.model, LatentAutoencoderDecoderOnly)
    ...
    def _filter_gaussians_by_opacity(
        self, recon: Gaussian, threshold: float
    ) -> Optional[Gaussian]:
        opacity = recon.get_opacity
        if opacity is None:
            return recon
        mask = opacity.squeeze(-1) >= threshold
        if mask.sum() == 0:
            return None
        if mask.all():
            return recon
        filtered = Gaussian(device=recon.device, **recon.init_params)
        filtered._xyz = recon._xyz[mask].detach()
        filtered._features_dc = recon._features_dc[mask].detach()
        if recon._features_rest is not None and recon._features_rest.numel() > 0:
            filtered._features_rest = recon._features_rest[mask].detach()
        else:
            filtered._features_rest = None
        filtered._scaling = recon._scaling[mask].detach()
        filtered._rotation = recon._rotation[mask].detach()
        filtered._opacity = recon._opacity[mask].detach()
        return filtered
