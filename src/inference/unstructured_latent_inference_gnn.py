...
from src.models.gnn_refine_original import SceneGraphRefiner
...
class SceneGraph2UnstructuredLatentPipeline:
    ...
    def load_model(self):
        ...
        self.model.load_state_dict(
            torch.load(
                "pretrained/training_recon_gnn/cluster/overfitting/regularize-epoch-7500.pth.tar",
                map_location="cpu" if not torch.cuda.is_available() else "cuda",  
            )["model"]
        )
    ...
    def compute_sparsity(vol, threshold=0.1):
        coords = torch.nonzero(vol[:, -1] > threshold, as_tuple=False)
        num_active_voxels = coords.shape[0]
        total_voxels = vol.shape[0] * vol.shape[2] * vol.shape[3] * vol.shape[4]
        sparsity_fraction = num_active_voxels / total_voxels
    ...
    def save_render(self, reconstruction, scene_ids, means, scales):
        if not osp.exists("vis/rendered_gnn_registered_7500"):
            os.makedirs("vis/rendered_gnn_registered_7500")
        ...
        for i, frame_id in enumerate(poses):
            ...
            frame_path = f"vis/rendered_gnn_registered_7500/{scene_id}_frame_{i:03d}.png"
        ...
        video_path = f"vis/rendered_gnn_registered_7500/{scene_id}_rendered.mp4"
        imageio.mimsave(video_path, rendered_frames, fps=30)

    def save_render_orbit(
        self, reconstruction, scene_ids, means, scales, output_dir="vis/rendered_gnn_registered_7500"
    ):
    ...
    def save_render_orbit_gs(
        self, scene_ids, obj_ids, means, scales, output_dir="vis/rendered_gnn_registered_7500_gs"
    ):
        os.makedirs(output_dir, exist_ok=True)
        scene_id = scene_ids[0]
